...
export default async function executeSecondPhase(
  user: User,
  matterId: string,
  ...
  if ((user.objectFiles?.length ?? 0) > 0) {
    ...
    if (
      (mailArchiveInfo as archiveData).status !== completeText ||
      (driveArchiveInfo as archiveData).status !== completeText
    ) {
...
async function getRequestedArchiveInfo(
  matterId: string,
  exportId: string,
): Promise<archiveData | string> {
...
async function downloadContents(
  ...
  const url = `https://storage.googleapis.com/storage/v1/b/${bucketName}/o/${objectName}?alt=media`;
...
async function uploadObject(
  ...
  const res = await axios
    .put(url, file, {
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Range': `bytes ${range}/${contentLength}`,
      },
    })
    .then(function (res) {})
    .catch(function (err) {
